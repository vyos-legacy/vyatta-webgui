...
function UTM_confDNS(name, callback, busLayer)
{
    ...
    this.f_validate = function()
    {
        ...
        if (thisObj.m_form.conf_dns_auto.checked) {
            thisObj.m_dnsObj.m_mode = 'auto';
            return true;
        } else {
            thisObj.m_dnsObj.m_mode = 'manual';
        }
        
        if (!thisObj.f_checkIP(thisObj.m_form.conf_dns_primary.value)) {
            if (!thisObj.f_checkHostname(thisObj.m_form.conf_dns_primary.value)) {
                errorInner += thisObj.f_createListItem(g_lang.m_dns_Primary + ' ' + g_lang.m_dns + ' ' + g_lang.m_formInvalid);
            }
        }
        
        if (thisObj.m_form.conf_dns_secondary.value.trim().length > 0) {
            if (!thisObj.f_checkIP(thisObj.m_form.conf_dns_secondary.value)) {
                if (!thisObj.f_checkHostname(thisObj.m_form.conf_dns_secondary.value)) {
                    errorInner += thisObj.f_createListItem(g_lang.m_dns_Secondary + ' ' + g_lang.m_dns + ' ' + g_lang.m_formInvalid);
                }
            }
        }
