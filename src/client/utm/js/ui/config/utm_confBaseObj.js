...
function UTM_confBaseObj(name, callback, busLayer)
{
    ...
    this.f_createGridHeader = function(header, onclick)
    {
        ...
        for (var i = 0; i < header.length; i++) {
            ...
            var rBorder = (i == header.length - 1) || h[0].length < 2 ? '' : 'border-right:1px solid #CCC; ';
	...
	this.f_renderTextfield = function(val, elId, readonly, size)
	{		
		var tf = '<input id="' + elId + '" type="text"' +
		       /*' class="v_form_input"' + */ ' size="' + size + '"';
